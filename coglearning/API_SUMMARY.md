...
ثبت‌نام	POST	/api/accounts/register/	ایجاد کاربر با نقش (Student/Teacher)
...
۲. مدیریت آزمون‌ها - پنل استاد (Assessment - Teacher)
...
۳. فرآیند شرکت در آزمون - شهروند (Assessment - Student)
...
مدیریت محتوا (استاد)	POST	/api/adaptive-learning/teacher/content/create/	تعیین بازه min_level و max_level برای محتوا
...
داشبورد استاد	GET	/api/analytics/teacher-dashboard/	آمار محتواها، تعداد آزمون‌های در انتظار تصحیح
...
امنیت سطح: تمام APIهای دانش‌آموز (بجز تعیین سطح) توسط پرمیشن HasTakenPlacementTest محافظت می‌شوند.
...
سوالات تشریحی: بعد از پایان آزمون توسط دانش‌آموز، وضعیت سشن pending_review می‌ماند تا استاد نمره بدهد؛ پس از آن سطح کاربر آپدیت می‌شود.
