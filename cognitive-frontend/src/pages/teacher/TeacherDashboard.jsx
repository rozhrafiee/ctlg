    ...
    <div className="space-y-6">
      ...
      <div className="surface p-6 border-neutral-200/80">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <Input
            placeholder="شناسه یا نام کاربری شهروند"
            value={studentId}
        ...
        {report && (
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">شناسه گزارش</div>
              <div className="text-lg font-semibold text-neutral-900">{report.id ?? '-'}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">شناسه شهروند</div>
              <div className="text-lg font-semibold text-neutral-900">{report.user ?? '-'}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">میانگین حافظه</div>
              <div className="text-2xl font-bold text-primary">{report.avg_memory_score ?? 0}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">میانگین تمرکز</div>
              <div className="text-2xl font-bold text-secondary">{report.avg_focus_score ?? 0}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">میانگین منطق</div>
              <div className="text-2xl font-bold text-accent-dark">{report.avg_logic_score ?? 0}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4">
              <div className="text-xs text-neutral-500">تعداد آزمون‌های تکمیل‌شده</div>
              <div className="text-2xl font-bold text-neutral-900">{report.total_tests_completed ?? 0}</div>
            </div>
            <div className="rounded-xl border border-primary/10 bg-primary-soft/30 p-4 md:col-span-2">
              <div className="text-xs text-neutral-500">آخرین به‌روزرسانی</div>
              <div className="text-lg font-semibold text-neutral-900">{formatDateTime(report.last_updated)}</div>
            </div>
          </div>
        )}
      </div>
